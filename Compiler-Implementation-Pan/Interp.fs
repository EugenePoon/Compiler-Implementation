...
(* 函数环境列表
  ...
  [ ("main",
   ([(TypI, "i")],
    Block
      [Dec (TypI,"r");
       statement (Expr (Call ("fac",[Access (AccVar "i"); Addr (AccVar "r")])));
       statement (Expr (Prim1 ("printi",Access (AccVar "r"))))]))]

函数环境 是 多态类型  'data env ---(string * 'data ) list 的一个 具体类型 ⭐⭐⭐
    类型变量 'data  具体化为  (paramdecs * statement)
    (string * (paramdecs * statement)) list
*)

type funEnv = (paramdecs * statement) env
...
let rec allocate (typ, x) (env0, nextloc) sto0 : locEnv * store =
    ...
    let (nextloc1, v, sto1) =
        ...
        | TypArray (t, Some i) -> (nextloc + i, nextloc, initSto nextloc i sto0)
        // 常规变量默认值是 0
        | _ -> (nextloc, 0, sto0)
...
let rec exec statement (locEnv: locEnv) (gloEnv: gloEnv) (store: store) : store =
    match statement with
    | If (e, statement1, statement2) ->
        let (v, store1) = eval e locEnv gloEnv store

        if v <> 0 then
            exec statement1 locEnv gloEnv store1 //True分支
        else
            exec statement2 locEnv gloEnv store1 //False分支
    ...
    | For(dec, e, opera,body) -> 
        ...
        //求值 循环条件,注意变更环境 store
            let (v, store2) = eval e locEnv gloEnv store1
              // 继续循环
            if v<>0 then  let (res ,store3) = eval opera locEnv gloEnv (exec body locEnv gloEnv store2)
                          loop store3
                    else store2  
        loop store0
    ...
    | Block statements ->
        ...
        let rec loop ss (locEnv, store) =
            ...
            | s1 :: sr -> loop sr (statementordec s1 locEnv gloEnv store)

        loop statements (locEnv, store)

    | Return _ -> failwith "return not implemented" // 解释器没有实现 return

and statementordec statementordec locEnv gloEnv store =
...
and eval e locEnv gloEnv store : int * store =
    ...
    | UnaryPrim (ope, e1) ->
        ...
        let (res,store) =
            match ope with
            | "!" -> if i1 = 0 then (1,store1) else (0,store1)
            | "printi" ->
                (printf "%d " i1
                 (i1,store1))
            | "printc" ->
                (printf "%c" (char i1)
                 (i1,store1))
            | "I++" -> 
                let ass = Assign (tmp e1,BinaryPrim ("+",Access (tmp e1),CstInt 1))
                let (i2, store2) = eval ass locEnv gloEnv store1
                (i1,store2)
            | "++I" -> 
                let ass = Assign (tmp e1,BinaryPrim ("+",Access (tmp e1),CstInt 1))
                let (i2, store2) = eval ass locEnv gloEnv store1
