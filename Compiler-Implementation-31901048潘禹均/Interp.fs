...
type funEnv = (paramdecs * IStatement) env
...
let rec allocate (IPrimitiveType, x) (env0, nextloc) sto0 : locEnv * store =

    let (nextloc1, v, sto1) =
        match IPrimitiveType with
        //数组 调用 initSto 分配 i 个空间
        | TypeArray (t, Some i) -> (nextloc + i, nextloc, initSto nextloc i sto0)
        // 常规变量默认值是 0
        | _ -> (nextloc, 0, sto0)

    msg $"\nalloc:\n {((IPrimitiveType, x), (env0, nextloc), sto0)}"
...
//初始化 解释器环境和store
let initEnvAndStore (TopDeclares: TopDeclare list) : locEnv * funEnv * store =
    ...
    let rec addv decs locEnv funEnv store =
        ...
        | VariableDeclare (IPrimitiveType, x) :: decr ->
            let (locEnv1, sto1) = allocate (IPrimitiveType, x) locEnv store
            addv decr locEnv1 funEnv sto1

        //全局函数 将声明(f,(xs,body))添加到全局函数环境 funEnv
        | FunctionDeclare (_, f, xs, body) :: decr -> addv decr locEnv ((f, (xs, body)) :: funEnv) store

    // ([], 0) []  默认全局环境
    // locEnv ([],0) 变量环境 ，变量定义为空列表[],下一个空闲地址为0
    // ([("n", 1); ("r", 0)], 2)  表示定义了 变量 n , r 下一个可以用的变量索引是 2
    // funEnv []   函数环境，函数定义为空列表[]
    addv TopDeclares ([], 0) [] emptyStore

(* ------------------------------------------------------------------- *)

(* Interpreting micro-C statements *)

let rec exec IStatement (locEnv: locEnv) (gloEnv: gloEnv) (store: store) : store =
    match IStatement with
    | If (e, IStatement1, IStatement2) ->
        let (v, store1) = eval e locEnv gloEnv store

        if v <> 0 then
            exec IStatement1 locEnv gloEnv store1 //True分支
        else
            exec IStatement2 locEnv gloEnv store1 //False分支
    ...
    | Block stmts ->
        ...
        let rec loop ss (locEnv, store) =
            ...
            | s1 :: sr -> loop sr (StatementORDeclare s1 locEnv gloEnv store)

        loop stmts (locEnv, store)

    | Return _ -> failwith "return not implemented" // 解释器没有实现 return

and StatementORDeclare StatementORDeclare locEnv gloEnv store =
    match StatementORDeclare with
    | Statement stmt -> (locEnv, exec stmt locEnv gloEnv store)
    | Declare (IPrimitiveType, x) -> allocate (IPrimitiveType, x) locEnv store
...
and eval e locEnv gloEnv store : int * store =
    match e with
    | Access acc ->
        let (loc, store1) = IAccess acc locEnv gloEnv store
    ...
    | ConstInt i -> (i, store)
    | Address acc -> IAccess acc locEnv gloEnv store
...
and IAccess acc locEnv gloEnv store : int * store =
    match acc with
    | AccessVariable x -> (lookup (fst locEnv) x, store)
    | AccessDeclareReference e -> eval e locEnv gloEnv store
    | AccessIndex (acc, idx) ->
        let (a, store1) = IAccess acc locEnv gloEnv store
...
// vs 参数列表 [8,2,...]
// 可以为空 []
let run (Prog TopDeclares) vs =
    //
    let ((varEnv, nextloc), funEnv, store0) = initEnvAndStore TopDeclares
